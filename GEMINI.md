...
## Base Rules
- Use idiomatic Go with explicit error paths and minimal side effects per package.
- Accept `context.Context` as the first parameter on request-scoped I/O boundaries.
- Wrap errors with `%w` and use `errors.Is/As` for classification; avoid panic-driven control flow.
- Keep package APIs small and use interfaces only where they improve substitution clarity.
- Prefer consumer-defined interfaces and avoid abstractions that are not yet needed.
- Prefer table-driven tests for behavior matrices and edge cases.
...
- Run `gofmt -w` only when formatting is intended, then verify formatting expectations.
- Run `go vet ./...`.
- Run `go test ./...`.
- Run `go test -race ./...` when touching concurrent code paths.
